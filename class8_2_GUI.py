...
from Segmentation.class1_AWS_extractor import AWS_extractor
from Segmentation.class3_append_label import append_label
from Segmentation.class7_draw_picture import draw_picture
from Segmentation.class8_0_build_show_sample import Radar_features_append
from Segmentation.class9_evaluation import Evaluation
import os
from sklearn.externals import joblib
from sklearn.preprocessing import MinMaxScaler

RadarDIR = r'J:\2017radar\\' #所有的雷达数据目录
AWSDIR = r'J:\AWSdata十年数据' #所有自动站数据所在目录
Radar_time = 0
trainData = 0
...
def generateDataExample():
    global trainData, Radar_time, RadarDIR
    draw = draw_picture()
...
# 进行预测
def predict():
    global trainData, preDictImage,Radar_time,RadarDIR
    # img_open = Image.open(
...
def generateReal():
    global realWindImage,realFlag,Radar_time, AWSDIR
    ...
    #
    # AWS_data =[]
    # AWS_path = os.path.join(AWSDIR, str(Radar_time)[:-4])
    # files = os.listdir(AWS_path)
    # for f in files:
    #     if int(f.split('_')[1]) == Radar_to_AWS_time(Radar_time):
    #         AWS_data = AWS_extractor().get_AWS_record(AWS_path,wind_speed=0)
    #         print("AWS_data.shape:", AWS_data.shape)
    #
    # draw = draw_picture()
    # AWS_wind_pic = draw.draw_AWS_wind_pic(composite_r,Radar_time,colors,AWS_data)
    # statestr.set('真实大风图片展示如下：')
    # img_png = ImageTk.PhotoImage(AWS_wind_pic)
    # label_img.config(image=img_png)
    # label_img.image = img_png
...
def selectPath():
    global originImagePath, Radar_time
    path_ = askopenfilename()
    path.set(path_)
    pathstr = pathentry.get()
    print("pathstr:", pathstr)
    if pathstr=='':
        return
    filename = pathstr.split('/')[-1]
    components = filename.split('_')
    time_height = components[2]+'_'+ components[3]
    Radar_time = int(components[2])
    imageName.set(time_height+'.bmp') #显示图片名称

    image = np.fromfile(pathstr, dtype=np.uint8).reshape(700, 900)
    originImagePath = lxc.drawboard(image,lxc.COLORS,time_height)
