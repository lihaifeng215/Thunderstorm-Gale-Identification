...
def Add_draw(resultPath,imagePath,windThred):
    ...
    with Image.open(imagePath).convert('RGBA') as im:
        ...
        savePath = './myFigures/predict%s.bmp'%name
        out.save(savePath)
        ...
        with Image.open(imagePath).convert('RGBA') as im:
            ...
            out = Image.alpha_composite(im, lx)
            savePath = './myFigures/predictNEW%s.bmp' % name
            out.save(savePath)
        return savePath
        #out.show((y-7, x-7, y+7, x+7),None, 'red')
def getPointsSet(startPoint,makeRec,startInd):
    ...
    return pointsSet
# 05081806 05082006
sp = Add_draw(r'reg_dataset_to_draw_pic.csv',r'./myFigures/201705040506_2500.bmp',19)
